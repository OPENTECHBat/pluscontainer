            ...
            if (this.props.action.params.picking_id && this.props.action.params.picking_type_id) {
                ...
                for (let i = 0; i < this.display_parts_list.length; i++) {
                    ...
                    this.display_parts_list[i].forEach((part) => {
                        if (part.genuine_stock_move && part.data.quantity > part.data.qty_done) {
                            list1.push(part)
                        } else if (part.genuine_stock_move && part.data.quantity <= part.data.qty_done) {
                            list2.push(part)
...
//        });
        ...
        if (scanned_part[0].tracking === 'none') {
            ...
            if (indexes_of_scanned_product_moves.length === 1) {
                ...
                if (!stock_move.genuine_stock_move) {
                    ...
                    if (stock_move.data.quantity > stock_move.data.qty_done) {
                        stock_move.is_selected = true
                        stock_move.data.qty_done += quantity
                        this.scanned_order_ids.push(scanned_part[0].id)
                    } else if (stock_move.data.quantity <= stock_move.data.qty_done) {
                        display_parts_list_copy.unshift(
                            {
                                id: this.line_id,
                                is_selected: true,
    ...
    handleClickTotalQuantity = (stock_move) => {
        ...
        if (display_parts_list_copy[selected_stock_move_id_index].id === stock_move.id) {
            stock_move.data.qty_done += stock_move.data.quantity - stock_move.data.qty_done
...
//core.action_registry.add('warehouse_picking_operations', WarehouseOperation);
actionRegistry.add('warehouse_picking_operations', WarehouseOperation);
