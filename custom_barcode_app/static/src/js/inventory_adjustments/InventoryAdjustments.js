...
import {useService,useBus, useListener} from "@web/core/utils/hooks";
import {Component} from "@odoo/owl";
import {Mutex} from "@web/core/utils/concurrency";
//import core from 'web.core';
import { registry } from "@web/core/registry";
//const {useState, useExternalListener, useRef, useEffect, onWillStart, onWillUnmount} = owl;
import {useState, useExternalListener, useRef, useEffect, onWillStart, onWillUnmount} from "@odoo/owl";
...
//        core.bus.off('barcode_scanned', this, this.scan1);
    ...
    getDataUpdateState = async () => {
        let data= await this.orm.call("stock.quant.line.prepare", "get_inventory_adjustment_lines", [[]]);
...
//        });
    ...
    getInventoryAdjustmentDataLocation = async (product_id, location_id) => {
        let data = await this.orm.call("stock.quant.line.prepare", "get_inventory_adjustment_data_location",  [[], product_id, location_id]);
...
//        });
    ...
    applyInventory = async (ids) => {
        let data =  await this.orm.call("stock.quant", "action_apply_inventory",  [ids]);
...
//        });
    ...
    saveBarcodeData = async (prepared_inventory_lines) => {
        let ids =  await this.orm.call("stock.quant", "save_barcode_data",  [[], prepared_inventory_lines]);
...
actionRegistry.add('inventory_adjustments', InventoryAdjustments);